...
cmdl_parser.add_argument('-radius_spherical_mask', type=int, required=False,
						help="Radius of spherical mask enclosing the particle ")
...
def main():
	...
	if args.model_map is not None:
		...
		if args.mask is None: #if no mask is given, generate a circular one
			...
			mask.to_zero();	
			if args.radius_spherical_mask is None:
				sphere_radius = np.min(sizeMap) // 2;
			else:
				sphere_radius = args.radius_spherical_mask;
