...
simple_cmd = 'python -em emmap.mrc -mm modmap.mrc -ma mask.mrc -p 1.0 -w 10 -o scaled.mrc'

cmdl_parser = argparse.ArgumentParser(
                 prog=sys.argv[0],
                 description='*** Perform local amplitude scaling with FDR based error control ***',
...
cmdl_parser.add_argument('-locResMap', '--locResMap', help='Input filename local-resolution map')
...
def get_central_scaled_pixel_vals_after_scaling(emmap, modmap, masked_xyz_locs, wn, apix, meanNoise, varNoise):
    ...
    mean_noise_radial_average = np.mean(np.asarray(noise_radial_average))
    ...
    for k, j, i in (masked_xyz_locs - wn / 2):
        ...
        mean_emmap_radial_average = np.mean(np.asarray(emmap_radial_average))
		
        ratio_radial_averages = mean_noise_radial_average/ mean_emmap_radial_average

        mod_radial_average = compute_radial_amplitude_distribution(modmap_wn, apix)
        mod_radial_average_array = np.asarray(mod_radial_average) 
        weighted_mod_radial_average_array = mod_radial_average_array*ratio_radial_averages
        weighted_mod_radial_average = weighted_mod_radial_average_array.tolist()
        map_b_sharpened = set_radial_amplitude_distribution(emmap_wn,  mod_radial_average, apix)
        mapNoise_sharpened = set_radial_amplitude_distribution(noiseMap, weighted_mod_radial_average, apix)
...
def run_window_function_including_scaling(emmap, modmap, mask, wn, apix, meanNoise, varNoise):
    """
    >>> emmap, modmap, mask = setup_test_data()
    ...
    sharpened_vals, sharpened_mean_vals, sharpened_var_vals = get_central_scaled_pixel_vals_after_scaling(emmap, modmap, masked_xyz_locs, wn, apix, meanNoise, varNoise)
...
def prepare_mask_and_maps_for_scaling(args):
    ...
    #if wished so, do local filtration of model map                    
    if args.locResMap is not None:
        locResMap = get_image(args.locResMap)
        modmapData, _, _ = localFiltration(modmap, locResMap, args.apix, False, wn)
        modmap = EMNumPy.numpy2em(modmapData)
...
def run_window_function_including_scaling_mpi(emmap, modmap, mask, wn, apix, meanNoise, varNoise):
    """
    >>> emmap_name, modmap_name, mask_name = setup_test_data_to_files()
    >>> import subprocess
...
def launch_amplitude_scaling(args):    	
    ...
    meanNoise, varNoise = estimateNoiseFromMap(EMNumPy.em2numpy(emmap), wn)
    splitFilename = os.path.splitext(os.path.basename(args.outfile))
	

    if not args.mpi:
        LocScaleVol, meanVol, varVol = run_window_function_including_scaling(emmap, modmap, mask, wn, args.apix, meanNoise, varNoise)
        ...
        qVolData = calcQMap(LocScaleVolData, meanVolData, varVolData, maskData, 'BY', 'rightSided')
        qVolData = np.subtract(np.ones(qVolData.shape), qVolData)
        qVol = EMNumPy.numpy2em(qVolData)

        #write the volumes    
        LocScaleVol = write_out_final_volume_window_back_if_required(args, wn, window_bleed_and_pad, LocScaleVol, args.outfile)
        qVol = write_out_final_volume_window_back_if_required(args, wn, window_bleed_and_pad, qVol, splitFilename[0] + '_qMap.mrc')
        ...
        makeDiagnosticPlot(emmap, wn, wn)

    elif args.mpi:
        LocScaleVol, meanVol, varVol, rank = run_window_function_including_scaling_mpi(emmap, modmap, mask, wn, args.apix, meanNoise, varNoise)
        ...
        if rank == 0:
			...
			#write the volumes
            LocScaleVol = write_out_final_volume_window_back_if_required(args, wn, window_bleed_and_pad, LocScaleVol, args.outfile)
            qVol = write_out_final_volume_window_back_if_required(args, wn, window_bleed_and_pad, qVol, splitFilename[0] + '_qMap_inv.mrc')
            meanVol = write_out_final_volume_window_back_if_required(args, wn, window_bleed_and_pad, meanVol, 'meanVol.mrc')
            varVol = write_out_final_volume_window_back_if_required(args, wn, window_bleed_and_pad, varVol, 'varVol.mrc') 
