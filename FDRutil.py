...
def calcQMap(map, mean, var, mask, method, test):
	...
	if np.isscalar(var):
		...
		var[var==0] = 1000; #just to avoid division by zero
		map = np.subtract(map, mean); 	
	...
	pMapRight = 1.0 - (0.5*(1.0 + special.erf(tMap/math.sqrt(2.0)))); 
	pMapLeft = (0.5*(1.0 + special.erf(tMap/math.sqrt(2.0))));
	...
	binaryMask[ binaryMask ==  0] = np.nan;
...
def printSummary(args, time):
	...
	if args.model_map is not None:
		...
		print(output);
		output = "Output LocScale qMap: " + splitFilename[0] + "_locscale_confidenceMap" + ".mrc";
		print(output);
	else:
		if args.locResMap is not None:
			splitFilename = os.path.splitext(os.path.basename(args.em_map));
			...
			print(output);
			output = "Output qMap: " + splitFilename[0] + "_confidenceMap" + ".mrc";
			print(output);
		else:
			splitFilename = os.path.splitext(os.path.basename(args.em_map));
			output = "Output qMap: " + splitFilename[0] + "_confidenceMap" + ".mrc";
