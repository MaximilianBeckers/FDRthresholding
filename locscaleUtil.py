...
def get_central_scaled_pixel_vals_after_scaling(emmap, modmap, masked_xyz_locs, wn, apix, locFilt, locResMap, boxCoord):
    ...
    for k, j, i in (masked_xyz_locs - wn / 2):
        ...
        if locFilt == True:
            tmpRes = locResMap[k, j, i]
            mapNoise_sharpened = mapNoise_sharpened.process("filter.lowpass.tanh", {"cutoff_abs": tmpRes.item(), "fall_off": 0.1})
            map_b_sharpened = map_b_sharpened.process("filter.lowpass.tanh", {"cutoff_abs": tmpRes.item(), "fall_off": 0.1})     
...
def prepare_mask_and_maps_for_scaling(args):
    ...
    if args.locResMap is not None:
        ...
        locResMapData = np.divide(args.apix, locResMapData)		
        locFilt = True		
    else:
        locFilt = False	
        locResMapData = EMNumPy.numpy2em(np.ones([xsize, ysize, zsize]))
