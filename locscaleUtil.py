...
def get_central_scaled_pixel_vals_after_scaling(emmap, modmap, masked_xyz_locs, wn, apix, meanNoise, varNoise):
    ...
    noiseMapData = emmapData[int(center[0]-0.5*wn):(int(center[0]-0.5*wn) + wn), int(0.02*sizeMap[1]+wn):(int(0.02*sizeMap[1]+wn) + wn), (int(center[2]-0.5*wn)):(int((center[2]-0.5*wn) + wn))]
	...
	#mean_noise_radial_average = np.max(np.asarray(noise_radial_average))
...
def prepare_mask_and_maps_for_scaling(args):
    ...
    #if wished so, do local filtration of the model map                     
    ...
    return emmap, modmap, mask, wn, window_bleed_and_pad, FDRmethod
...
def launch_amplitude_scaling(args):    	
    ...
    startTime = time.time()
    emmap, modmap, mask, wn, window_bleed_and_pad, FDRmethod = prepare_mask_and_maps_for_scaling(args)
    ...
    if not args.mpi:
        ...
        if rank == 0:
            ...
            #make diagnostic plot and print summary
            makeDiagnosticPlot(emmap, wn, wn, True)	
